...
- Selecting a model switches it. On a patched OpenCode build this is a true
  one-click switch (see [One-click switching](#one-click-switching)); on stock
  OpenCode it opens the native picker.
...
| Filter              | just type                                     |
| Move selection      | `up` / `down` or `ctrl+p` / `ctrl+n`          |
| Page                | `pageup` / `pagedown`                         |
| Clear filter        | `ctrl+u`                                      |
| Switch to model     | `enter` or click a row                        |
| Leave search        | `escape`                                      |
